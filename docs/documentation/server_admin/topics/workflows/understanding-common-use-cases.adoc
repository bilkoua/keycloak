...
When a new user is created, a workflow can automatically assign initial roles or groups to the user and send them an invitation email with a one-time link to activate their account. This ensures that new users have the necessary access from the moment they join.
...
If you only need a simple notification without an action link, you can use the `notify-user` step instead:
